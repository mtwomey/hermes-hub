...
**The blunt read (updated 2026-09-01):** hermes-hub has the transport, per-spoke authorization, artifact handling, and W3 in-session tool surface required for the product path. V10 M1–M3 additionally prove Pumpkin's local hub and spoke run as supervised launchd services rather than hand-started foreground processes. The remaining V10 uncertainty is deliberately narrow: M4 cold-boot/login survival must be verified by Matthew after reboot.
