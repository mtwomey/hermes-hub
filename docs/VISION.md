...
| V10 | Local Hermes is a spoke too | **Pumpkin's own Hermes auto-connects to the hub as a spoke**, alongside Olive. **V10 M1–M4 verified 2026-09-01:** Pumpkin's hub and local spoke run as separate supervised `ai.hermes.hub` / `ai.hermes.spoke` LaunchAgents, recover from direct kills, and automatically start and reconnect after a human-verified reboot. Evidence: `.hermes/evidence/v10-m4-cold-boot.md`. | Uniform model: the desktop session reaches every machine the same way, including its own. No special-casing "local." Also means no manual process starting (see V1). |
...
|---|---|---|---|
| In-session model tools | ✅ 6 tools (`peer_ask`, `peer_list`, `peer_info`, `peer_discover`, `peer_status`, `peer_fetch_artifact`) | ✅ **live in-session, bidirectional (W4b)** — all six tools ported (V14), packaged as a native plugin (`plugin/hermes_hub_peer/`) registering via `PluginContext.register_tool()` with **no** `.pth` and **no** Hermes core patch, installed on **both** Pumpkin and Olive 2026-09-02. Proven end to end: handlers verified live against a real hub + real Hermes spoke (gate 1), loadable in an isolated runtime with an end-to-end call through the registered handler (gate 2), and — after install + gateway restart — **the model called `peer_list` unprompted in a real desktop session** from the tool descriptions alone, with no tool named in the request (M3, Pumpkin side). **W4b extended this to Olive**: in a real conversation running ON Olive, `peer_list` reports Pumpkin, a natural-language request naming no tool caused Olive's model to select `peer_ask` itself, and re-entrancy (Pumpkin asks Olive; Olive's agent asks Pumpkin back mid-task) was tested for the first time and completed cleanly with no deadlock. V3 cache safety measured: the real system prompt is byte-identical (`aa8d7f55…`) whether a spoke is connected or not. Evidence: `.hermes/evidence/w3-gate1.md`, `w3-gate2.md`, `w3-m3-live-install.md`, `w4b-m2-olive-tools.md`, `w4b-m3-round-trip.md`. | **Required (V1)** |
...
| Loads into the live session | ✅ `.pth` + gateway shim | ✅ **installed and loading** — native plugin at `plugin/hermes_hub_peer/`, symlinked into `~/.hermes/plugins/`, loads with **no** Hermes core patch and **no** `.pth`. Verified 2026-09-01: gateway restarted (PID 26837 → 74101) and the tools were live in the next session. | **Required (V1/V10)** |
| Survives Olive's firewall | ❌ inbound blocked | ✅ solved, proven | **Required (V2)** |
| Per-peer authorization | ✅ per-peer Keychain tokens | ✅ **spoke-enforced per-peer credentials, genuinely distinct (V5a, fixed W4b)** — opaque credential travels hub-relayed (never validated/stored/logged by the hub) and spoke-checked via `hmac.compare_digest` before any agent invocation. Verified live: wrong/missing credential fails with the agent never invoked; correct credential succeeds; hub log grepped for the credential value with 0 matches. See `.hermes/plans/2026-09-01_100000-w1-w2-auth-and-artifacts.md` Gate 1, `.hermes/evidence/w1-gate1.md`. **W4b fixed a real defect found on the way to W4b M2: every installed credential value — across both machines — was the SAME shared secret, making per-peer authorization decorative despite the code being correct.** `assert_distinct_credential_topology()` (TDD, fails against a shared-secret fixture) now exists to catch this class of bug; all values were rotated to four genuinely independent secrets, and the decisive attack (`caller:Olive:credential` authenticating as `spoke:Pumpkin:credential`) was proven to now fail (`task rejected`, no agent invocation). See `.hermes/evidence/w4b-m1-credential-distinctness.md`. | **Required (V5)** |
...
**The blunt read (updated 2026-09-02, W4b):** hermes-hub has the transport, per-spoke authorization, artifact handling, and in-session tool surface required for the product path, **now bidirectional**. **W4 Gates 1–3 passed:** a normal Pumpkin conversation selected `peer_ask` for a natural-language request, Olive read and returned its real `cldsi-dev-support-dbt` pipeline README as an artifact, and Pumpkin verified the source/destination SHA-256 end to end. **M4 then passed with Matthew performing the reboots:** both spokes automatically reappeared, and an authenticated post-reboot `peer_ask` reached Olive and returned its hostname/user. The task used Olive's direct outbound LAN WebSocket to Pumpkin's hub; no firewall change was made. Evidence: `.hermes/evidence/w4-m1-olive-credentials.md`, `w4-m3-real-task.md`, and `w4-m4-reboot-survival.md`.

**W4b closed the two gaps W4 left open.** First, a real defect: every credential across both machines — the hub's external token, both spoke/caller pairs — held the exact same shared-secret value, making per-peer authorization decorative (V5's design was correct and enforced; the installed *values* were wrong). W4's own completion matrix had read matching hashes as proof of correct pairing, which was only half right — a pair matching is necessary but not sufficient; unrelated accounts must also differ, and nothing checked that. W4b added `assert_distinct_credential_topology()` (TDD, fails against a shared-secret fixture) and rotated all values to four genuinely independent secrets, one spoke at a time, with a decisive negative proof that `caller:Olive:credential` no longer authenticates as `spoke:Pumpkin:credential` (`task rejected`, no agent invocation, Olive's own spoke log confirms). Second, V1's goal was still only half delivered: Olive could be asked, but could not ask. W4b installed the `hermes-hub-peer` plugin, Olive's own LAN-correct `~/.hermes-hub/config.json`, and its two required credentials (never the caller key for itself — a machine has no reason to hold its own command key) on Olive, restarted only Olive's gateway and spoke (never Pumpkin's gateway), and verified from a real conversation running ON Olive: `peer_list` reports Pumpkin, a natural-language request naming no tool caused Olive's model to select `peer_ask` itself, and Pumpkin's spoke log recorded `credential accepted, invoking agent` for the resulting task. Re-entrancy — a peer calling back into the peer that called it, mid-task — was reachable for the first time and was tested directly: it completed cleanly with no deadlock, evidenced independently on both machines' own logs. Evidence: `.hermes/evidence/w4b-m1-credential-distinctness.md`, `w4b-m2-olive-tools.md`, `w4b-m3-round-trip.md`, `w4b-completion-matrix.md`.
