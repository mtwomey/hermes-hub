...
|---|---|---|---|
| In-session model tools | ✅ 6 tools (`peer_ask`, `peer_list`, `peer_info`, `peer_discover`, `peer_status`, `peer_fetch_artifact`) | ✅ **live in-session** — all six tools ported (V14), packaged as a native plugin (`plugin/hermes_hub_peer/`) registering via `PluginContext.register_tool()` with **no** `.pth` and **no** Hermes core patch, and installed 2026-09-01. Proven end to end: handlers verified live against a real hub + real Hermes spoke (gate 1), loadable in an isolated runtime with an end-to-end call through the registered handler (gate 2), and — after install + gateway restart — **the model called `peer_list` unprompted in a real desktop session** from the tool descriptions alone, with no tool named in the request (M3). V3 cache safety measured: the real system prompt is byte-identical (`aa8d7f55…`) whether a spoke is connected or not. Evidence: `.hermes/evidence/w3-gate1.md`, `w3-gate2.md`, `w3-m3-live-install.md`. **Caveat:** V10 M1–M4 have verified the supervised hub + Pumpkin spoke through a human-performed reboot; the next product acceptance gate is W4's real cross-machine file-transfer task. | **Required (V1)** |
...
**The blunt read (updated 2026-09-02):** hermes-hub has the transport, per-spoke authorization, artifact handling, and in-session tool surface required for the product path. **W4 Gates 1–3 passed:** a normal Pumpkin conversation selected `peer_ask` for a natural-language request, Olive read and returned its real `cldsi-dev-support-dbt` pipeline README as an artifact, and Pumpkin verified the source/destination SHA-256 end to end. **M4 then passed with Matthew performing the reboots:** both spokes automatically reappeared, and an authenticated post-reboot `peer_ask` reached Olive and returned its hostname/user. The task used Olive's direct outbound LAN WebSocket to Pumpkin's hub; no firewall change was made. Evidence: `.hermes/evidence/w4-m1-olive-credentials.md`, `w4-m3-real-task.md`, and `w4-m4-reboot-survival.md`.
