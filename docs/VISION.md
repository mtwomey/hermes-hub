...
|---|---|---|---|
| In-session model tools | ✅ 6 tools (`peer_ask`, `peer_list`, `peer_info`, `peer_discover`, `peer_status`, `peer_fetch_artifact`) | 🟡 **built and packaged, install pending** — all six tools ported (V14) and proven live against a real hub + real Hermes spoke, then packaged as a native plugin (`plugin/hermes_hub_peer/`) that registers them through `PluginContext.register_tool()` with **no** `.pth` and **no** Hermes core patch. Proven loadable in an isolated runtime (subprocess, temp `HERMES_HOME`): all six land in the real tool registry and an end-to-end `peer_ask` through the registered handler returned a real agent answer. V3 cache safety measured, not asserted: the real system prompt is byte-identical (`aa8d7f55…`) whether a spoke is connected or not. **Not yet in-session** — installing into `~/.hermes/plugins/` and restarting the gateway is W3 M3, reserved for Matthew; see `docs/INSTALL-PLUGIN.md`. Evidence: `.hermes/evidence/w3-gate1.md`, `w3-gate2.md`. | **Required (V1)** |
...
| Loads into the live session | ✅ `.pth` + gateway shim | 🟡 **native plugin built, not installed** — `plugin/hermes_hub_peer/`, loadable without touching Hermes core; install + gateway restart is M3 (Matthew's) | **Required (V1/V10)** |
